package biz.flatsw.cabledesigner.model;

public class OverMaximumException extends RuntimeException {
    private final Pin pin;
    private final float crossSection;
    private final float insulationDiameter;

    public OverMaximumException(Pin pin, float crossSection, float insulationDiameter) {
        this.pin = pin;
        this.crossSection = crossSection;
        this.insulationDiameter = insulationDiameter;
    }

    public Pin getPin() {
        return pin;
    }

    public float getCrossSection() {
        return crossSection;
    }

    public float getInsulationDiameter() {
        return insulationDiameter;
    }

    public String toString() {
        return "Over maximum: "+pin.getConnector().getName()+"/"+pin.getName()+", cross="+crossSection+", diameter="+insulationDiameter;
    }
}
