...
public class ConnectorPinSectionImpl implements ConnectorPinSection {
    ...
    @Override
    public ConnectorPinComponent findMatchingPin(WireType wireType) {
        return pinTypes
                .stream()
                .filter(type -> type.suits(wireType))
                .findFirst()
                .orElse(null);
    }
    ...
    @Override
    public ConnectorPinComponent findMatchingSeal(WireType wireType) {
        return pinSeals
                .stream()
                .filter(type -> type.suits(wireType))
                .findFirst()
